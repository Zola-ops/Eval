...
def flatten_result(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(sc_kp_1_list, list):
        flat["scorer_key_point_1"] = " | ".join(str(k) for k in sc_kp_1_list) if sc_kp_1_list else ""
    else:
        flat["scorer_key_point_1"] = str(sc_kp_1_list) if sc_kp_1_list else ""

    # 评分的 key_point_0 数组
    sc_kp_0_list = sc.get("key_point_0", [])
    if isinstance(sc_kp_0_list, list):
        flat["scorer_key_point_0"] = " | ".join(str(k) for k in sc_kp_0_list) if sc_kp_0_list else ""
    else:
        flat["scorer_key_point_0"] = str(sc_kp_0_list) if sc_kp_0_list else ""

    # key_point_1_result 数组：1分考点判定结果
    ...
    if isinstance(kpr_1_list, list):
        ...
        for kpr in kpr_1_list:
            ...
            evidence = kpr.get("evidence", "")
            lines.append(f"【1分考点】{point}\n【是否满足】{satisfied}\n【判定依据】{evidence}")
        flat["keypoint_1_results"] = "\n\n".join(lines)
        # 统计
        total = len(kpr_1_list)
        satisfied_count = sum(1 for k in kpr_1_list if str(k.get("satisfied", "")).lower() in ("true", "1", "是", "满足"))
        flat["keypoint_1_results_summary"] = f"{satisfied_count}/{total}"
    else:
        flat["keypoint_1_results"] = json.dumps(kpr_1_list, ensure_ascii=False) if kpr_1_list else ""
        flat["keypoint_1_results_summary"] = ""

    # key_point_0_result 数组：0分考点判定结果
    ...
    if isinstance(kpr_0_list, list):
        ...
        for kpr in kpr_0_list:
            ...
            evidence = kpr.get("evidence", "")
            lines.append(f"【0分考点】{point}\n【是否满足】{satisfied}\n【判定依据】{evidence}")
        flat["keypoint_0_results"] = "\n\n".join(lines)
    else:
        flat["keypoint_0_results"] = json.dumps(kpr_0_list, ensure_ascii=False) if kpr_0_list else ""
...
def main():
    ...
    parser.add_argument('--api_key', type=str, help='智谱API Key')
    parser.add_argument('--api_url', type=str, help='智谱API URL')
    parser.add_argument('--config', type=str, default='评估配置.json',
                       help='配置文件路径')
    ...
    evaluator = Evaluator(
        ...
        api_token=args.api_key or "21dbddd1dbc943b6884c750bf26b311c.enht4roYNGtZbhfT"
    )
